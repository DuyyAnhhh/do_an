        ...
        <div class="container">
            <div class="flex-w flex-sb-m p-b-52">
                <div class="flex-w flex-l-m m-tb-10 m-t-30 m-b-15">
                    <a href="{{ route('page.product.brand') }}">
                        <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5 {{ Request::get('id') == null ? 'how-active1' : '' }}">
                            Tất cả
                        </button>
                    </a>

                    @if (isset($trademarks) && count($trademarks) > 0)
                        @foreach($trademarks as $key => $trademark)
                            <a href="{{ route('page.product.brand') }}?id={{ $trademark->id }}">
                                <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5 {{ Request::get('id') == $trademark->id ? 'how-active1' : '' }}" data-filter=".{{ $trademark->td_name }}" >
                                    {{ $trademark->td_name }}
                                </button>
                            </a>
                        @endforeach
